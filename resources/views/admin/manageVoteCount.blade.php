...
@section('content')
    ...
    @php
        ...
        $liveStatus = HomeSection::where('title', 'ElectionLiveSection')->value('status') ?? 0;
    @endphp
                            ...
                            <div class="row pt-4 px-3">
                                <div class="checkbox-wrapper-46">
                                    <div class="d-flex align-item-center">
                                        <form action="{{ route('toggle.live.section') }}" method="POST" class="d-inline">
                                            @csrf
...
@endif --}}
                        ...
                        <form action="{{ route('voteSave') }}" method="post" id="voteForm">
                            @csrf
                                    ...
                                    <tbody>
                                        @foreach ($results as $party)
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="number" class="form-control seat-input" placeholder="W+L"
                                                        name="{{ strtolower($party->abbreviation) }}_wl"
                                                        value="{{ old(strtolower($party->abbreviation) . '_wl', $party->total_seats ?? 0) }}">
                                                </td>
                                                <td>
                                                    <input type="number" class="form-control" placeholder="W"
                                                        name="{{ strtolower($party->abbreviation) }}_w"
                                                        value="{{ old(strtolower($party->abbreviation) . '_w', $party->seats_won) }}">
                                                </td>
                                                <td>
                                                    <input type="number" class="form-control" placeholder="L"
                                                        name="{{ strtolower($party->abbreviation) }}_l"
                                                        value="{{ old(strtolower($party->abbreviation) . '_l', $party->seat_loss) }}">
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
